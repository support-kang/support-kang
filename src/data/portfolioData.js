export const projectRecords = [
  {
    ...
    company: '에스비티글로벌',
    problem:
      '주문, 가격, 출하, 할당 업무가 Salesforce 화면과 SAP 시스템에 분산되어 있어, 승인된 변경 사항만 운영 시스템에 안전하게 반영하고 각 처리 단계의 상태를 추적할 수 있는 구조가 필요했다.',
    decision:
      '주요 업무를 하나의 Salesforce 화면에서 처리할 수 있도록 통합하고, SAP 연동은 비동기 처리 구조로 분리했다. 요청 접수, 외부 시스템 전송, 처리 결과 반영을 각각 독립적으로 관리해 장애 발생 시 실패 구간을 식별하고 재처리할 수 있도록 설계했다.',
    outcome:
      '가격, 주문, 출하, 할당 변경 사항이 승인 절차를 거쳐 Salesforce와 SAP에 일관되게 반영되도록 핵심 연동 기능을 구축했다. 또한 각 요청의 진행 상태와 처리 결과를 추적할 수 있어 운영 안정성과 데이터 정합성을 높였다.',
    keyFeatures: [
      {
        title: '가격·특가 승인 및 SAP 가격 조건 연동',
        detail:
          '다단계 승인 완료 후 SAP에 가격 조건을 생성하고, Salesforce 상품 가격 정보까지 자동 반영했다. 승인 상태와 외부 시스템 처리 상태를 분리해 일부 단계만 완료된 건도 추적할 수 있도록 구성했다.',
      },
      {
        title: '주문 수량·저장 위치·배치 정보 통합 변경',
        detail:
          '주문 수량, 저장 위치, 배치 변경 요청을 하나의 비동기 처리 구조로 통합했다. 출하 진행 중 저장 위치가 변경되는 경우에는 재주문 처리와 중복 실행 방지 로직을 적용해 데이터 충돌을 차단했다.',
      },
      {
        title: '출하 예약 검증 및 SAP 연동',
        detail:
          '출하 채널, 잔여 수량, 출하 가능 용량을 검증한 뒤 예약 데이터를 원자적으로 반영하고 SAP로 전송했다. 사용자 요청 접수와 SAP 처리 완료 상태를 분리해 운영 중에도 진행 상황과 실패 지점을 확인할 수 있도록 구현했다.',
      },
      {
        title: '할당 수량 실시간 조회 및 재배분',
        detail:
          'SAP의 최신 할당 수량을 실시간으로 조회하고, 전체 수량이 유지되는 범위에서 주문 라인 간 수량을 재배분하도록 구현했다. 처리 중인 요청에 대한 중복 변경도 제한해 데이터 정합성을 확보했다.',
      },
    ],
  },
  {
